...
strGroup = Request.QueryString("summary")
...
cmd.CommandText = "dccps.cancercomm_pkg.GetSummary"
cmd.Parameters.Append cmd.CreateParameter("groupfield", adVarchar, adParamInput, 25, strGroup)
set rst = cmd.Execute
%>
...
%>
    <tr>
      <td><%
            ' The Total row should link to the complete listing, all other rows should link
            ' to the details page.
            If "Total" = rst(strGroup) Then
	        strUrl = "cancercomm.asp"
	    Else
	        strUrl = "details.asp?param=" & strGroup & "&value=" & rst(strGroup)
	    End If
	%><a href="<%=strUrl%>"><%=rst(strGroup)%></a></td>
      <td><%=rst("num_grants")%></td>
      <td><%=rst("nci_dollars")%></td>
    </tr>
<%
    rst.MoveNext
Loop
