...
Sub DisplayGrants(strProgram, strBranch, strActive)
    ...
    Set rst = RetrieveDataDynamic(numgrants, strProgram, strBranch, vbNullString, vbNullString, vbNullString, strActive, 0)

    DisplayGrantsAll rst, numgrants, strProgram, strBranch, strActive, False
End Sub
...
' Notes:	CR#38313
'***********************************************************************
Sub DisplayGrantsByCode(strProgram, strBranch, strCodeName, strActive)
    Dim rst
    Dim numgrants

    Set rst = RetrieveDataDynamic(numgrants, strProgram, strBranch, vbNullString, vbNullString, strCodeName, strActive, 0)
...
Sub DisplayGrantsMechanism(strProgram, strBranch, strMechanism, intFY)
    ...
    Set rst = RetrieveDataDynamic(numgrants, strProgram, strBranch, vbNullString, strMechanism, vbNullString, strActive, intFY)
...
Sub DisplayGrantsRFA(strRFA, strActive)
    ...
    Set rst = RetrieveDataDynamic(numgrants, vbNullString, vbNullString, _
				"IN (" & strRFA & ")", vbNullString, vbNullString, strActive, 0)

    DisplayGrantsAll rst, numgrants, vbNullString, vbNullString, strActive, False
End Sub
...
Sub DisplayGrantsNoRFA(strProgram)
	...
	Set rst = RetrieveDataDynamic(numgrants, strProgram, vbNullString, "IS NULL", vbNullString, vbNullString, "ACTIVE", 0)

	DisplayGrantsAll rst, numgrants, strProgram, vbNullString, "ACTIVE", False
End Sub
...
Sub DisplayFullGrantsInPortfolio (strOwner, strPortfolioName)
	...
	DisplayGrantsAll rst, numgrants, vbNullString, vbNullString, "ALL", False
End Sub
...
' Notes:	CR# 9235
'***********************************************************************
Sub DisplayGrantsInPortfolioByCode ( strOwner, strPortfolioName, strCodeName )
	Dim rst
	Dim numgrants

	Set rst = RetrieveDataPortfolioByCode ( strOwner, strPortfolioName, strCodeName )
	numgrants = GetGrantCountPortfolioByCode ( strOwner, strPortfolioName, strCodeName )
	DisplayGrantsAll rst, numgrants, vbNullString, vbNullString, "ALL", False
End Sub
...
' Notes:	CR# 9235
'***********************************************************************
Sub DisplayGrantsInPortfolioByCode_LP ( strOwner, strPortfolioName, strBranch, strCodeName )
	Dim rst
	Dim numgrants

	Set rst = RetrieveDataPortfolioByCode ( strOwner, strPortfolioName, strCodeName )
	numgrants = GetGrantCountPortfolioByCode ( strOwner, strPortfolioName, strCodeName )
	DisplayGrantsAll rst, numgrants, vbNullString, strBranch, "ALL", False
End Sub
...
' Notes:	CR# 17929
'***********************************************************************
Sub DisplayGrantsInPortfolioHistory(strOwner, strPortfolioName)
	Dim rst
	Dim numgrants

	Set rst = RetrieveDataPortfolioHistory(strOwner, strPortfolioName)
	numgrants = GetGrantCountPortfolioHistory(strOwner, strPortfolioName)
...
'***********************************************************************
'***********************************************************************
Sub DisplayGrantsInPortfolio (strOwner, strPortfolioName)
	Dim rst
	Dim numgrants

	Set rst = RetrieveDataPortfolio(strOwner, strPortfolioName)

	numgrants = GetGrantCountPortfolio(strOwner, strPortfolioName)
	DisplayGrantsAll rst, numgrants, vbNullString, vbNullString, "ALL", False
End Sub

'***********************************************************************
'***********************************************************************
Sub DisplayGrantsInPortfolio_LP (strOwner, strPortfolioName, strBranch)
	Dim rst
	Dim numgrants
...
'***********************************************************************
Sub DisplayGrantsByPI (strFirstName,strLastName,strOrderBy, bDisplayFY)
	...
	DisplayGrantsAll rst, numgrants, vbNullString, vbNullString, "ALL", bDisplayFY
End Sub
...
' Notes:	CR# 9235
'***********************************************************************
Function RetrieveDataPortfolioByCode ( strOwner, strPortfolioName, strCodeName )
    Dim cmd
    Dim strSPName
    Dim strError
...
Function RetrieveDataPortfolio(strOwner, strPortfolioName)
    Dim cmd
    Dim strSPName
    Dim strError
...
Function RetrieveDataPortfolioHistory(strOwner, strPortfolioName)
    Dim cmd
    Dim strSPName
    Dim strError
...
Function RetrieveDataDynamic(ByRef numgrants, strProgram, strBranch, strRFA, strMechanism, strCodeName, strActive, intFY)
    Dim strWhere
    Dim cmd
    ...
    ' Chilisoft and/or Oracle has a bug that is triggered when passing numeric input parameters.
    ' Use varchar to work around this problem.
    ' 2/19/2002 -- This problem seems to have been fixed, so we need to use adNumeric again.
    ...
    numgrants = GetGrantCount(strProgram, strBranch, strRFA, strMechanism, strCodeName, strActive, intFY)
...
' HF (1/21/09) CR #41982 - display fy, is so specified.
Sub DisplayGrantsAll(rst, numgrants, strProgram, strBranch, strActive, bDisplayFY)
    dim strName
    dim strTitle
    Dim iSerialNum
    Dim strURL
    ...
    If rst.EOF Then
		...
		Response.write "<p>&nbsp;&nbsp;&nbsp;<font face=""Arial, sans-serif"">No grants have been found for this category.</font></p>" & vbCrLf
	Exit Sub
    End If

    MakeCounts intStart, intCount

    strFY = GetParameterValue("fy")
    blnHasFY = Not IsNull(strFY) And vbNullString <> strFY

    strPortfolio = GetParameterValue("portfolio")
...
%>
    ...
    <% If Not blnHasFY and Not blnHasPortfolio and "ARCHIVE" <> strActive Then
    %>
    <p><font face="Arial, Helvetica, sans-serif">This list contains grants carried over from prior fiscal years and new grants awarded through fiscal year 2008.</font></p>
    <% End If %>
   ...
   <table cellpadding="4" cellspacing="0" align="center" border="1" bordercolor="999999" width="95%">
    <tr>
...
<%
	If (bDisplayFY = True) Then
		Response.Write("<th id=""FY"" valign=""top"" align=""center""><font face=""Arial, sans-serif"">Fiscal<br>Year</font></th>")
	End If
%>
...
%>
    <tr>
      <td headers="PI Name and Organization" valign="top" align="left">
...
--->
          <%=BuildName(rst("last_name"), rst("first_name"), rst("mi_name"))%><br>
          <%=rst("org_name")%>
        </font></td>

      <td headers="Project Title / Link to Abstract" valign="top" align="left">
        <font size="-1" face="Arial, sans-serif">
      <%
        strTitle = rst("project_title")
      	If rst("abstract_exists_code") = "Y" Then
      %><A HREF = "/grants/abstract.asp?ApplID=<%=rst("APPL_ID")%>"><%=strTitle%></A><%
        Else
      %><%=strTitle%><%
        End If
      %>
      </font></td>
      <td headers="Grant Number / Link to Publications" valign="top" align="left" nowrap>
        <font size="-1" face="Arial, sans-serif">
      <%
       ...
       strURL="http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=search&db=PubMed&tool=NCIDCCPS&dispmax=20&term=" _
				...
				End If
      %>
         <a href="<%=strURL%>"><%=rst("full_grant_num")%></a>
        </font>
       </td>
<%
	If (bDisplayFY = True) Then
		Response.Write("<td>" & rst("fy") & "</td>")
	End If
%>
...
<%
  ...
  <a name="pubmed"></a>
<!---
   CR#37054 - nf 01.17.2006  - Re\move PubMed link around PI Name
   CR# 16457 - L. Tsao 07.22.2004 Re\moved size attribute for font to make sizes consistent. --
...
Function GetGrantCount(strProg, strBranch, strRFA, strMechanism, strCodeName, strActive, intFY)
    Dim strWhere
    Dim cmd
...
Function GetFullGrantCountPortfolio(strOwner, strPortfolioName)
    ...
    cmd.Parameters.Append cmd.CreateParameter("owner", adVarChar, adParamInput, 8, strOwner)
...
' Notes:	CR# 9235
'***********************************************************************
Function GetGrantCountPortfolioByCode ( strOwner, strPortfolioName, strCodeName )
    Dim strWhere
    Dim cmd
...
Function GetGrantCountPortfolioHistory(strOwner, strPortfolioName)
    Dim strWhere
    Dim cmd
...
' Returns the number of grants that will be returned for a particular search result.
' Builds and passes in a where clause to the GetGrantCount stored procedure.
Function GetGrantCountPortfolio(strOwner, strPortfolioName)
    Dim strWhere
    Dim cmd
...
Function MakeCounts(intStart, intCount)
		...
		strStart=GetParameterValue("start")
		strCount=GetParameterValue("count")
...
'---------------------------------------------------------------
'Calls a SP in the DB to find the max length of a program abrv
Function ProgMatches (strProgram, intMaxProgLen)
    Dim cmd

    Set cmd = GetValidationCommand("FindProgramMatch", vbNullString)
    cmd.Parameters.Append cmd.CreateParameter("p_program_abrv", adVarChar, adParamInput, intMaxProgLen, strProgram)
...
'---------------------------------------------------------------
'Calls a SP in the DB to find the max length of a program abrv
Function BranchMatches (strBranch, intMaxBranchLen)
    Dim cmd

    Set cmd = GetValidationCommand("FindBranchMatch", vbNullString)
    cmd.Parameters.Append cmd.CreateParameter("p_branch_abrv", adVarChar, adParamInput, intMaxBranchLen, strBranch)
...
'			count
'***********************************************************************
Function GetPINamesAndGrantCounts(category, errString)
